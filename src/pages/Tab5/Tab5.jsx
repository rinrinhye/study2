  ...
  return (
    <>
      <div className="mx-auto w-[500px] my-20 flex justify-between gap-20">
        {["open layer 1", "open layer 2", "open layer 3"].map((label, idx) => {
          const id = `button-${idx}`;
          const isExpanded = modal.isOpen && activeButtonRef.current.id === id;
      ...
      {modal.isOpen && <LayerSearch onClose={closeLayer} />}
    </>
  );
}
